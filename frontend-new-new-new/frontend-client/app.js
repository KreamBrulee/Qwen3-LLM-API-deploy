...
loginFormEl.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (authOverlayEl) authOverlayEl.style.display = "";
    showAuthError(err.message.includes("401") ? "Invalid credentials." : err.message);
  } finally {
    btn.classList.remove("loading", "success");
    btn.innerHTML = '<span>Sign In</span><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1 7h12M8 2l5 5-5 5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  }
});
...
registerFormEl.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    btn.classList.remove("loading", "success");
    btn.innerHTML = '<span>Create Account</span><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1 7h12M8 2l5 5-5 5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  }
});
...
async function onLoginSuccess(data) {
  ...
  // ── Cinematic login animation sequence ──
  await playLoginAnimation(data.username);
...
function drawPixelChar(ctx, scale) {
  // 12x16 pixel grid, drawn at scale
  // Color palette: skin=#f5c179, hair=#d4845a(fluffy), clothes=#2a2a2a/#3355aa
  const S = scale;
  ...
  // -- Fluffy hair (rows 0-3) --
  // Row 0: top fluff
  px(3,0,'#e8a86a'); px(4,0,'#d4845a'); px(5,0,'#e8a86a'); px(6,0,'#d4845a'); px(7,0,'#e8a86a'); px(8,0,'#d4845a');
  // Row 1: wide fluff
  px(2,1,'#d4845a'); px(3,1,'#e8a86a'); px(4,1,'#c97040'); px(5,1,'#e8a86a'); px(6,1,'#c97040'); px(7,1,'#e8a86a'); px(8,1,'#c97040'); px(9,1,'#d4845a');
  // Row 2: hair over forehead
  px(2,2,'#c97040'); px(3,2,'#e8a86a'); px(4,2,'#d4845a'); px(5,2,'#e8a86a'); px(6,2,'#d4845a'); px(7,2,'#e8a86a'); px(8,2,'#d4845a'); px(9,2,'#c97040');
  // Row 3: hair meets face
  px(2,3,'#c97040'); px(3,3,'#d4845a'); px(4,3,'#f5c179'); px(5,3,'#f5c179'); px(6,3,'#f5c179'); px(7,3,'#f5c179'); px(8,3,'#d4845a'); px(9,3,'#c97040');

  // -- Face (rows 4-7) --
  // Row 4
  px(3,4,'#f5c179'); px(4,4,'#f5c179'); px(5,4,'#f5c179'); px(6,4,'#f5c179'); px(7,4,'#f5c179'); px(8,4,'#f5c179');
  // Row 5 - eyes
  px(3,5,'#f5c179'); px(4,5,'#222222'); px(5,5,'#f5c179'); px(6,5,'#f5c179'); px(7,5,'#222222'); px(8,5,'#f5c179');
  // Row 6
  px(3,6,'#f5c179'); px(4,6,'#f5c179'); px(5,6,'#f5c179'); px(6,6,'#f5c179'); px(7,6,'#f5c179'); px(8,6,'#f5c179');
  // Row 7 - mouth
  px(3,7,'#f5c179'); px(4,7,'#f5c179'); px(5,7,'#c06030'); px(6,7,'#c06030'); px(7,7,'#f5c179'); px(8,7,'#f5c179');

  // -- Neck (row 8) --
  px(5,8,'#f5c179'); px(6,8,'#f5c179');

  // -- Body/jacket (rows 9-12) --
  // Row 9
  px(2,9,'#222222'); px(3,9,'#222222'); px(4,9,'#222222'); px(5,9,'#222222'); px(6,9,'#222222'); px(7,9,'#222222'); px(8,9,'#222222'); px(9,9,'#222222');
  // Row 10
  px(2,10,'#222222'); px(3,10,'#333333'); px(4,10,'#333333'); px(5,10,'#222222'); px(6,10,'#222222'); px(7,10,'#333333'); px(8,10,'#333333'); px(9,10,'#222222');
  // Row 11
  px(2,11,'#222222'); px(3,11,'#222222'); px(4,11,'#3355aa'); px(5,11,'#3355aa'); px(6,11,'#3355aa'); px(7,11,'#3355aa'); px(8,11,'#222222'); px(9,11,'#222222');
  // Row 12
  px(2,12,'#222222'); px(3,12,'#222222'); px(4,12,'#3355aa'); px(5,12,'#3355aa'); px(6,12,'#3355aa'); px(7,12,'#3355aa'); px(8,12,'#222222'); px(9,12,'#222222');

  // -- Legs (rows 13-15) --
  // Row 13
  px(3,13,'#3355aa'); px(4,13,'#3355aa'); px(7,13,'#3355aa'); px(8,13,'#3355aa');
  // Row 14
  px(3,14,'#3355aa'); px(4,14,'#3355aa'); px(7,14,'#3355aa'); px(8,14,'#3355aa');
  // Row 15 - feet
  px(2,15,'#333333'); px(3,15,'#333333'); px(4,15,'#333333'); px(7,15,'#333333'); px(8,15,'#333333'); px(9,15,'#333333');
}

// ── Login animation orchestrator ─────────────────────
function playLoginAnimation(username) {
  return new Promise((resolve) => {
    ...
    const percentEl = document.getElementById("pixelPercent");
    const charCanvas = document.getElementById("pixelCharCanvas");
    const authCard = document.querySelector(".auth-card-wide");
    const mainArea = document.getElementById("mainArea");

    // Stage 1: Auth card exit animation (0ms)
    if (authCard) authCard.classList.add("exit");
    setTimeout(() => {
      authOverlayEl.classList.add("exit");
    }, 200);

    // Stage 2: Show pixel loading screen (400ms)
    setTimeout(() => {
      authOverlayEl.style.display = "none";
      authOverlayEl.classList.remove("exit");
      // Reveal the main shell only once the auth overlay is actually hidden.
      document.body.classList.add("logged-in");
      if (authCard) authCard.classList.remove("exit");
      if (splash) splash.style.display = "";


      // Hide retro background game & HUD during loading screen
      const retroGame = document.getElementById("retroBgGame");
      const retroHud  = document.querySelector(".retro-hud");
      if (retroGame) retroGame.style.display = "none";
      if (retroHud)  retroHud.style.display  = "none";

      // Draw pixel character
      if (charCanvas) {
        const scale = 6;
        charCanvas.width = 12 * scale;
        charCanvas.height = 16 * scale;
        const ctx = charCanvas.getContext("2d");
        ctx.clearRect(0, 0, charCanvas.width, charCanvas.height);
        drawPixelChar(ctx, scale);
      }
      ...
      function animateProgress(now) {
        ...
        // ease-out curve so it feels punchy at first then slows
        ...
        if (pct !== lastPct) {
          ...
          if (splashSub) {
            if (pct < 40) splashSub.textContent = "Loading...";
            else if (pct < 80) splashSub.textContent = "Preparing workspace...";
            else if (pct < 100) splashSub.textContent = "Almost ready...";
            else splashSub.textContent = "Let's go!";
          }
        }
    ...
    // Stage 3: Exit splash, reveal dashboard (2400ms)
    setTimeout(() => {
      if (splash) splash.classList.add("exit");
      if (mainArea) mainArea.classList.add("entrance");
    }, 2400);

    // Stage 4: Cleanup (3100ms)
    setTimeout(() => {
      if (splash) {
        ...
        if (splashSub) splashSub.textContent = "Loading...";
      }
      // Restore retro background game & HUD
      const retroGame = document.getElementById("retroBgGame");
      const retroHud  = document.querySelector(".retro-hud");
      if (retroGame) retroGame.style.display = "";
      if (retroHud)  retroHud.style.display  = "";
      if (mainArea) {
        setTimeout(() => mainArea.classList.remove("entrance"), 600);
      }
      resolve();
    }, 3100);
  });
...
function initSplashParticles() {
  ...
  const colors = ["#3b82f6", "#6366f1", "#06b6d4", "#a78bfa", "#10b981", "#f59e0b"];
